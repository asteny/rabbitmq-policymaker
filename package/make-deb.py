...
virtualenv[
    "-p", "/home/travis//virtualenv/python3.7/bin/python3", ENV_PATH
] & plumbum.FG
...
pip[
    ...
    "--no-binary=:all:",
    "-U",
    "git+https://github.com/asteny/rabbitmq-policymaker",
] & plumbum.FG

ln[
    "-snf",
    os.path.join(ENV_PATH, "bin", "rabbitmq_policymaker"),
    "/usr/bin/%s" % pkg_name,
] & plumbum.BG
...
args = (
    ...
    "deb",
    "--iteration",
    os.getenv("ITERATION", "0"),
    "-n",
    pkg_name,
    "--deb-systemd",
    "contrib/{}.service".format(pkg_name),
    "-v",
    version,
    "-p",
    "package",
    ...
    "libcap-ng0",
    "--config-files",
    "/etc/default/{}".format(pkg_name),
    "--config-files",
    "/etc/{}.conf".format(pkg_name),
)
...
deb_files = list(
    filter(
        ...
        map(lambda x: os.path.join("package/", x), os.listdir("package/")),
    )
)
